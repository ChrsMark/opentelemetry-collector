...
var MetricsInfo = metricsInfo{
	...
	V1SystemCPUUtilization: metricInfo{
		Name: "v1.system.cpu.utilization",
	},
}
...
type metricV1SystemCPUUtilization struct {
	data     pmetric.Metric // data buffer for generated metric.
	config   MetricConfig   // metric config provided by user.
	capacity int            // max observed number of data points added to the metric.
}

// init fills v1.system.cpu.utilization metric with initial data.
func (m *metricV1SystemCPUUtilization) init() {
	m.data.SetName("system.cpu.utilization")
...
func (m *metricV1SystemCPUUtilization) recordDataPoint(start pcommon.Timestamp, ts pcommon.Timestamp, val float64) {
	if !m.config.Enabled {
		return
	}
	dp := m.data.Sum().DataPoints().AppendEmpty()
...
// updateCapacity saves max length of data point slices that will be used for the slice capacity.
func (m *metricV1SystemCPUUtilization) updateCapacity() {
	if m.data.Sum().DataPoints().Len() > m.capacity {
		m.capacity = m.data.Sum().DataPoints().Len()
	}
}
...
// emit appends recorded metric data to a metrics slice and prepares it for recording another set of data points.
func (m *metricV1SystemCPUUtilization) emit(metrics pmetric.MetricSlice) {
	if m.config.Enabled && m.data.Sum().DataPoints().Len() > 0 {
		m.updateCapacity()
		m.data.MoveTo(metrics.AppendEmpty())
		m.init()
...
func newMetricV1SystemCPUUtilization(cfg MetricConfig) metricV1SystemCPUUtilization {
	m := metricV1SystemCPUUtilization{config: cfg}
...
type MetricsBuilder struct {
	...
	metricV1SystemCPUUtilization     metricV1SystemCPUUtilization
}
...
func NewMetricsBuilder(mbc MetricsBuilderConfig, settings scraper.Settings, options ...MetricBuilderOption) *MetricsBuilder {
	mb := &MetricsBuilder{
		...
		metricV1SystemCPUUtilization:     newMetricV1SystemCPUUtilization(mbc.Metrics.V1SystemCPUUtilization),
	}
...
func (mb *MetricsBuilder) EmitForResource(options ...ResourceMetricsOption) {
	...
	mb.metricV1SystemCPUUtilization.emit(ils.Metrics())
...
func (mb *MetricsBuilder) RecordSystemCPUUtilizationDataPoint(ts pcommon.Timestamp, val float64) {
	...
	if ReceiverHostmetricsEmitV1SystemConventionsFeatureGate.IsEnabled() {
		mb.metricV1SystemCPUUtilization.recordDataPoint(mb.startTime, ts, val)
	}
}
...
// RecordV1SystemCPUUtilizationDataPoint adds a data point to v1.system.cpu.utilization metric.
func (mb *MetricsBuilder) RecordV1SystemCPUUtilizationDataPoint(ts pcommon.Timestamp, val float64) {
	mb.metricV1SystemCPUUtilization.recordDataPoint(mb.startTime, ts, val)
}
